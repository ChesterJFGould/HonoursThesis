...
\todo{This section is good, just change to be about thesis not proposal}
...
In the next two subsections we will present two different versions of this judgment, the syntax directed version which can be decided by only considering $\Gamma$, $e_a$, and $e_b$, and the type directed version which also needs to consider $e_t$.
...
Hstrip-owever, we don't know what type these new variables should be given in the $\eta$ cases.
...
\todo{elaborate on how $\eta$ works here}.
...
\todo{See what to write here after going back over intro}
