...
\todo{Overall: Apply feedback given by William}
\todo{Overall: Check using we instead of I}
\todo{Overall: Type-directed instead of type directed, same for syntax-directed}
\subsection{Proof Assistants}
\done\todo{This section is good, just change to be about thesis not proposal}
...
These systems have an advantage over others, as the programs and proofs about these programs are written in a common language \citep{Nordstrom1990}.
...
\done\todo{This section is good, just change to be about thesis not proposal}
\todo{Add example here where type checking depends on unit eta}
...
So in the true branch of \verb|int_or_string| we make the judgement $b : \verb|Bool| \vdash 3141 : (\verb|if true then Int else String|)$, and since we are reasoning up to symbolic execution this is equal to to $b : \verb|Bool| \vdash 3141 : \verb|Int|$.
...
Instead of capturing the execution of a program, give us additional equalities that we can glean from the type of a term.
...
\todo{Update to be inline with thesis instead of proposal}

Another problem with NbE interpreters, if and how they can handle ``type-directed" rules.
...
\citet{Chapman2005} describe an algorithm which supports type-directed rules by giving the type of the two normalized terms as a argument to the procedure deciding their equality.
...
We propose that the algorithm described by \citet{Chapman2005} will perform as well as a purely syntax-directed approach while allowing for theories with a stronger, type-directed, judgemental equality.
The rest of this thesis is split into two parts.
First we define exactly what these two approaches to judgemental equality are, and how an algorithm for deciding the type-direct equality can be derived from the bidirectional recipe.
...
\done\todo{This should become thesis statement stuff}
...
\todo{Previous section only covered equality in broad strokes. Be clear that here we are going clearly define the two approaches we want to compare.}
\todo{Explain normal forms. How we use them to decide the beta equalities and if something is a specific type constructor. And also how we then only define the equality judgement for normal forms.}
...
We first present both syntax and type-directed equalities for a small dependent type theory in which the two equalities differ only in presentation.
Then, to show that the type-directed equality supports more rules than the syntax-directed equality, we extend the theory with both the unit type and dependent pairs \todo{don't imply that dependent pairs are different in type directed equality, just that we want them to show off the recipe}.
...
Therefore we present the theory in a form very close to that of both smalltt implementations.
We also don't prove any theorems about this theory.
However, the theory we present is very similar to that proposed in \citet{altenkirch2010}.
...
We will use these to represent the types during type checking since we don't need to do anything to them to see if they are a type constructor.
For example, if we used all expression to represent types, we could have $\appE{(\lamE{x}{\piE{y}{\unittE}{\unittE}})}{\unitE}$ which we would need to reduce to $\piE{y}{\unittE}{\unittE}$ to know that it is a type constructor.
...
The synth judgment $\synthJ{\Gamma}{e}{e_t}$ means that the type $n_t$ can be inferred for the term $e$ under the context $\Gamma$.
...
The Conv \todo{add label and reference for Conv} rule uses the typed equality judgment $\tyEqJ{\Gamma}{n_a}{n_b}{n_t}$ which we deliberately haven't defined yet.
...
In \autoref{fig:base-syntax-directed-equality} we define the typed equality judgement by appealing to the syntax directed equality judgement $\stxEqJ{\Theta}{n_a}{n_b}$.
...
Therefore we use $\Theta$ in the syntax directed equality judgement instead of $\Gamma$, and use strip-types to turn a type context into a scope.
...
\done\todo{elaborate on how $\eta$ works here}
...
In \autoref{fig:base-type-directed-equality} we define the typed equality judgement in terms of the equality check judgment $\chkEqJ{\Gamma}{n_a}{n_b}{n_t}$ and the equality synth judgment $\synEqJ{\Gamma}{n_a}{n_b}{n_t}$.\done\todo{make sure that synth and check judgments terminology is introduced when discussing typing judgments}
